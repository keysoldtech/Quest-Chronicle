...
const classes = {
    Barbarian: { baseHp: 24, baseDamageBonus: 4, baseShieldBonus: 0, baseAp: 3, healthDice: 4, stats: { str: 4, dex: 1, con: 3, int: 0, wis: 0, cha: 1 }, description: "\"Unchecked Assault\" - Once per turn, the Barbarian can discard a drawn spell card to perform an attack that deals +6 damage, but they lose 2 Shield Points." },
    Cleric:    { baseHp: 20, baseDamageBonus: 1, baseShieldBonus: 3, baseAp: 2, healthDice: 3, stats: { str: 1, dex: 0, con: 2, int: 1, wis: 4, cha: 2 }, description: "\"Divine Aid\" - Once per turn, the Cleric can add 1d4 to an attack roll or saving throw." },
    Mage:      { baseHp: 18, baseDamageBonus: 1, baseShieldBonus: 2, baseAp: 2, healthDice: 2, stats: { str: 0, dex: 1, con: 1, int: 4, wis: 2, cha: 1 }, description: "\"Mystic Recall\" - Once per turn, the Mage can choose to draw an additional spell card." },
    Ranger:    { baseHp: 20, baseDamageBonus: 2, baseShieldBonus: 2, baseAp: 2, healthDice: 3, stats: { str: 1, dex: 4, con: 2, int: 1, wis: 3, cha: 0 }, description: "\"Focused Shot\" - Once per turn, if the DM's range roll is exactly the required number for a drawn spell, the attack deals double damage." },
    Rogue:     { baseHp: 18, baseDamageBonus: 3, baseShieldBonus: 1, baseAp: 3, healthDice: 2, stats: { str: 1, dex: 4, con: 1, int: 2, wis: 0, cha: 3 }, description: "\"Opportunist Strike\" - Once per turn, if the first spell card drawn is usable against an enemy in Close range, the Rogue deals an additional +2 damage." },
    Warrior:   { baseHp: 22, baseDamageBonus: 2, baseShieldBonus: 4, baseAp: 3, healthDice: 4, stats: { str: 3, dex: 2, con: 4, int: 0, wis: 1, cha: 1 }, description: "\"Weapon Surge\" - Once per turn, after drawing a spell card, the Warrior can discard that card to add +4 to their damage." },
};
...
const magicalAffixes = {
    ...
    suffixes: [
        { name: 'of Fortitude', tier: 1, bonuses: { hp: 5 }, types: ['armor'] },
        { name: 'of Vigor', tier: 2, bonuses: { hp: 10 }, types: ['armor'] },
        { name: 'of the Titan', tier: 3, bonuses: { hp: 15 }, types: ['armor'] },
        { name: 'of Striking', tier: 1, bonuses: { damageBonus: 1 }, types: ['weapon'] },
        { name: 'of Ruin', tier: 2, bonuses: { damageBonus: 2 }, types: ['weapon'] },
        { name: 'of Annihilation', tier: 3, bonuses: { damageBonus: 3 }, types: ['weapon'] },
...
const spellCards = [
    { name: "Acid Burst", type: "Spell", class: ["Mage", "Ranger"], category: "Damage", apCost: 1, effect: { type: "damage", dice: "1d6", target: "any-monster", description: "Deals 1d6 acid damage in a 5-foot radius sphere." } },
    { name: "Cinder Shot", type: "Spell", class: ["Mage", "Ranger"], category: "Damage", apCost: 1, effect: { type: "damage", dice: "1d10", target: "any-monster", description: "Deals 1d10 fire damage." } },
    { name: "Force Barrier", type: "Spell", class: ["Mage"], category: "Utility", apCost: 1, effect: { type: "utility", status: "Guarded", duration: 2, target: "self", description: "Increase your Shield Points by 5 until your next turn." } },
    ...
    { name: "Inspire Allies", type: "Spell", class: ["Cleric", "Ranger"], category: "Utility", apCost: 1, effect: { type: "utility", target: "all-explorers", statusToApply: { name: 'Inspired', type: 'stat_modifier', bonuses: { damageBonus: 2 }, duration: 2 }, description: "All explorers gain a +2 bonus to their attack rolls for 1 round." } },
...
const weaponCards = [
    { name: "Axechuck", type: "Weapon", class: ["Any"], apCost: 2, effect: { type: "damage", dice: "1d6", description: "Thrown (20/60), Special: Returning Edge - Returns to hand at end of turn (If thrown and hand free)." } },
    ...
    { name: "Bolt Sprinter", type: "Weapon", class: ["Any"], apCost: 2, effect: { type: "damage", dice: "1d8", description: "Ammunition, Loading, Special: Steady Aim - First attack on next turn deals +1d4 damage." } },
    ...
    { name: "Doomcleaver", type: "Weapon", class: ["Any"], apCost: 2, effect: { type: "damage", dice: "2d6", critBonusDice: "1d6", description: "Two-Handed, Heavy, Special: Savage Chop - Make 1 additional melee attack vs same target (Natural 20 on attack roll)." } },
    { name: "Duelist's Point", type: "Weapon", class: ["Any"], apCost: 2, effect: { type: "damage", dice: "1d8", description: "Finesse, Special: Opening Flourish - First successful attack deals +1d4 damage." } },
    { name: "Farstrike Bow", type: "Weapon", class: ["Any"], apCost: 2, effect: { type: "damage", dice: "1d8", description: "Ammunition, Heavy, Two-Handed, Special: Piercing Shot - +1 Attack Roll but ignore 1 point of target's Shield Bonus." } },
    { name: "Quick Blade", type: "Weapon", class: ["Any"], apCost: 1, effect: { type: "damage", dice: "1d6", description: "Finesse, Special: Fluid Motion - Can use Break Away for 0 AP (If make two attacks with this weapon on turn)." } },
    { name: "Shadowtooth", type: "Weapon", class: ["Any"], apCost: 1, effect: { type: "damage", dice: "1d4", description: "Finesse, Thrown (20/60), Special: Poison Ready - Advantage on attack roll when applying poison." } },
    { name: "Swiftflight Bow", type: "Weapon", class: ["Any"], apCost: 2, effect: { type: "damage", dice: "1d6", description: "Ammunition, Close-Range Penalty (-1d4 damage when attacking Close enemy)." } },
];
const armorCards = [
...
const allMonsters = [
    { name: "Phantom Light", type: "Monster", maxHp: 5, requiredRollToHit: 12, attackBonus: 2, ap: 1, effect: { type: "damage", dice: "1d8", description: "Incorporeal Movement. Proximity damage." } },
    { name: "Ember Flicker", type: "Monster", maxHp: 6, requiredRollToHit: 12, attackBonus: 4, ap: 2, effect: { type: "damage", dice: "1d10", description: "On miss: Ignite (DC 12 DEX save or catch fire, 1d4 fire/turn)." } },
    { name: "Pestie Prowler", type: "Monster", maxHp: 7, requiredRollToHit: 10, attackBonus: 8, ap: 2, effect: { type: "damage", dice: "1d6", description: "Sneaky Escape — Dex roll (DC 12) to flee if ≤ 3 HP." } },
    { name: "Pestie Pilferer", type: "Monster", maxHp: 8, requiredRollToHit: 12, attackBonus: 4, ap: 2, effect: { type: "damage", dice: "1d6+2", description: "Quick Feet (Can Break Away as a bonus action.)." } },
    { name: "Grotto Weaver", type: "Monster", maxHp: 10, requiredRollToHit: 12, attackBonus: 6, ap: 2, effect: { type: "damage", dice: "1d6", description: "Web Shot (DC 10 Strength save to escape)." } },
    { name: "Flutterwing Swarm", type: "Monster", maxHp: 10, requiredRollToHit: 12, attackBonus: 4, ap: 2, effect: { type: "damage", dice: "2d4", description: "Blind Flight (Immune to blindness conditions.)." } },
    { name: "Pestie Whisperer", type: "Monster", maxHp: 10, requiredRollToHit: 12, attackBonus: 3, ap: 2, effect: { type: "damage", dice: "1d4", description: "Tribal Magic (+2 attack for pesties)." } },
    { name: "Scale-kin Skulker", type: "Monster", maxHp: 10, requiredRollToHit: 12, attackBonus: 2, ap: 2, effect: { type: "damage", dice: "1d4+1", description: "Trap Master (Sets a trap at start of combat)." } },
    { name: "Essence Thief", type: "Monster", maxHp: 15, requiredRollToHit: 12, attackBonus: 4, ap: 2, effect: { type: "damage", dice: "2d6", description: "Incorporeal." } },
    { name: "Bone Archer", type: "Monster", maxHp: 18, requiredRollToHit: 12, attackBonus: 3, ap: 2, effect: { type: "damage", dice: "1d8", description: "Bone Resilience (Immune to poison and charm.)." } },
    { name: "Veiled Fanatic", type: "Monster", maxHp: 20, requiredRollToHit: 12, attackBonus: 5, ap: 2, effect: { type: "damage", dice: "1d4+3", description: "Unholy Fervor (Can re-roll a failed saving throw)." } },
    { name: "Striped Marauder", type: "Monster", maxHp: 23, requiredRollToHit: 12, attackBonus: 4, ap: 3, effect: { type: "damage", dice: "1d8+2", description: "Rampage (Make another attack on kill)." } },
    { name: "Segmented Horror", type: "Monster", maxHp: 24, requiredRollToHit: 12, attackBonus: 5, ap: 3, effect: { type: "damage", dice: "1d4+3", description: "Wall Climber." } },
    { name: "Stonegaze Wyrmlet", type: "Monster", maxHp: 24, requiredRollToHit: 12, attackBonus: 4, ap: 3, effect: { type: "damage", dice: "1d6+2", description: "Petrification Gaze (DC 12 CON save or petrified)" } },
    { name: "Caustic Sludge", type: "Monster", maxHp: 24, requiredRollToHit: 12, attackBonus: 3, ap: 2, effect: { type: "damage", dice: "1d8", description: "Split on Lightning." } },
    { name: "Sky Lurer", type: "Monster", maxHp: 27, requiredRollToHit: 12, attackBonus: 4, ap: 3, effect: { type: "damage", dice: "1d6+1", description: "Swooping Attack." } },
    { name: "Highway Scourge", type: "Monster", maxHp: 28, requiredRollToHit: 12, attackBonus: 6, ap: 3, effect: { type: "damage", dice: "2d6+3", description: "Shout (+1 attack for allies)." } },
    { name: "Ruined Sentinel", type: "Monster", maxHp: 33, requiredRollToHit: 13, attackBonus: 6, ap: 3, effect: { type: "damage", dice: "2d8", description: "Magic Resistance." } },
    { name: "Shadowmaw Alpha", type: "Monster", maxHp: 34, requiredRollToHit: 13, attackBonus: 5, ap: 3, effect: { type: "damage", dice: "2d6+2", description: "Pack Tactics." } },
    { name: "Stone Wing", type: "Monster", maxHp: 35, requiredRollToHit: 13, attackBonus: 5, ap: 3, effect: { type: "damage", dice: "1d6+3", description: "Stone Form." } },
    { name: "Haunted Cuirass", type: "Monster", maxHp: 41, requiredRollToHit: 13, attackBonus: 4, ap: 3, effect: { type: "damage", dice: "2d6", description: "Unyielding Form (Immune to poison, fear)." } },
    { name: "Greenskin Mauler", type: "Monster", maxHp: 44, requiredRollToHit: 14, attackBonus: 5, ap: 3, effect: { type: "damage", dice: "1d12+3", description: "Relentless Endurance (Survives at 1 HP)." } },
    { name: "Spectral Corruptor", type: "Monster", maxHp: 45, requiredRollToHit: 14, attackBonus: 6, ap: 3, effect: { type: "damage", dice: "3d6", description: "Incorporeal Passage." } },
    { name: "Axehorn Brute", type: "Monster", maxHp: 55, requiredRollToHit: 14, attackBonus: 7, ap: 4, effect: { type: "damage", dice: "2d12+4", description: "Labyrinthine Recall." } },
    { name: "Briar Witch", type: "Monster", maxHp: 59, requiredRollToHit: 14, attackBonus: 5, ap: 4, effect: { type: "damage", dice: "2d6", description: "Cackle of Madness (Confuse enemies)." } },
    { name: "Wrapped Ancient", type: "Monster", maxHp: 60, requiredRollToHit: 14, attackBonus: 5, ap: 4, effect: { type: "damage", dice: "2d6+3", description: "Undead Fortitude (Can drop to 1 HP)." } },
    { name: "Watery Charmer", type: "Monster", maxHp: 63, requiredRollToHit: 14, attackBonus: 6, ap: 4, effect: { type: "damage", dice: "1d6+3", description: "Captivating Song (Charmed)." } },
    { name: "Hill Oaf", type: "Monster", maxHp: 64, requiredRollToHit: 15, attackBonus: 6, ap: 4, effect: { type: "damage", dice: "2d8+4", description: "Powerful Blow (+2 damage)." } },
    { name: "Deathlord Marshal", type: "Monster", maxHp: 78, requiredRollToHit: 15, attackBonus: 7, ap: 4, effect: { type: "damage", dice: "3d6+4", description: "Unholy Endurance (Can rise with 10 HP)." } },
    { name: "Earth Colossus", type: "Monster", maxHp: 112, requiredRollToHit: 16, attackBonus: 8, ap: 5, effect: { type: "damage", dice: "2d10+5", description: "Immutable Form." } },
    { name: "Winter Juggernaut", type: "Monster", maxHp: 130, requiredRollToHit: 17, attackBonus: 9, ap: 5, effect: { type: "damage", dice: "3d12+6", description: "Icy Aura (Deals cold damage)." } },
    { name: "Hellfire Sovereign", type: "Monster", maxHp: 195, requiredRollToHit: 18, attackBonus: 9, ap: 6, effect: { type: "damage", dice: "4d6+5", description: "Hellish Regeneration." } },
    { name: "Rotwood Behemoth", type: "Monster", maxHp: 214, requiredRollToHit: 18, attackBonus: 8, ap: 6, effect: { type: "damage", dice: "3d8+4", description: "Rooted Horror (difficult terrain)." } },
];

const monsterTiers = {
    tier1: allMonsters.filter(m => m.maxHp <= 25),
    tier2: allMonsters.filter(m => m.maxHp > 25 && m.maxHp <= 60),
    tier3: allMonsters.filter(m => m.maxHp > 60),
};
